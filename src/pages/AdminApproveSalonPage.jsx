...
export default function AdminApproveSalonPage() {
  ...
  const handleVerifySalon = async () => {
    ...
    try {
      setActionLoadingId(selectedSalon.id)
      ...
      const res = await fetch(
        `${BASE_URL}/api/admin/owner/verify-salon/${selectedSalon.id}?${query.toString()}`,
        {
          method: 'PATCH',
        }
      )
      ...
      if (!res.ok) {
        throw new Error(typeof data === 'string' ? data : 'Failed to verify salon')
      }
      ...
      if (msg.toLowerCase().includes('admin not found')) {
        ...
        toast.error('Only admin can verify salons')
      } else if (msg.toLowerCase().includes('salon not found')) {
        toast.error('Salon not found')
      } else {
        toast.error(msg || 'Failed to verify salon')
      }
    } finally {
      setActionLoadingId('')
    }
      ...
      <div className='max-w-7xl mx-auto py-2 animate-fadeIn'>
        <div className='mb-6'>
          ...
          <p className='text-sm text-gray-500 mt-1'>
            Review unverified salons and approve them for public listing.
          </p>
        </div>
          ...
          <div className='p-5 border-b border-gray-100'>
            <div className='flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3'>
              <div>
                ...
                <p className='text-sm text-gray-500 mt-1'>
                  Search and verify newly added salons.
                </p>
              </div>
              ...
              <div className='flex flex-col sm:flex-row gap-3'>
                <div className='relative min-w-0 sm:min-w-[260px]'>
                  <input
                    type='text'
                    placeholder='Search by salon, city, owner or contact...'
                    value={searchTerm}
              ...
              filteredSalons.map((salon, index) => (
                ...
                >
                  ...
                  <div className='mt-4 flex flex-col gap-2'>
                    <button
                      onClick={() => openVerifyModal(salon)}
                      disabled={actionLoadingId === salon.id}
                    ...
                    {salon.mapLink && (
                      <a
                        ...
                        target='_blank'
                        rel='noreferrer'
                        className='h-11 rounded-2xl border border-gray-200 bg-white text-gray-700 font-medium hover:bg-gray-50 transition flex items-center justify-center'
                      >
                        View Location
                      </a>
                    )}
                ...
                {loading ? (
                  ...
                  filteredSalons.map((salon, index) => (
                    ...
                    >
                      ...
                      <td className='px-5 py-5 align-top'>
                        <p className='text-sm text-gray-700 leading-6 max-w-[260px] break-words'>
                          {salon.address || '-'}
                        </p>
                      </td>
                        ...
                        <div className='flex items-center gap-3'>
                          ...
                          {salon.mapLink && (
                            <a
                              ...
                              target='_blank'
                              rel='noreferrer'
                              className='h-11 px-4 rounded-2xl border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 font-medium transition flex items-center'
                            >
                              View Location
                            </a>
                          )}
              ...
              <div className='relative w-full max-w-lg bg-white rounded-[28px] shadow-2xl animate-scaleIn flex flex-col'>
                ...
                <div className='px-5 sm:px-6 py-5 space-y-4'>
                  <div className='rounded-2xl border border-gray-200 bg-gray-50 p-4 space-y-2'>
                    ...
                    <p className='text-sm text-gray-700 break-all'>
                      <span className='font-medium text-gray-900'>Owner ID:</span>{' '}
                      {selectedSalon?.salonOwnerId || '-'}
                    </p>
                    <p className='text-sm text-gray-700 break-words'>
                  ...
                  <p className='text-sm sm:text-base text-gray-600 leading-7'>
                    Are you sure you want to verify this salon?
                  </p>
                ...
                <div className='flex flex-col-reverse sm:flex-row justify-end gap-3 px-5 sm:px-6 py-4 border-t border-gray-100'>
                  ...
                  >
                    {actionLoadingId ? 'Verifying...' : 'Verify Salon'}
                  </button>
                </div>
              </div>
            </div>
